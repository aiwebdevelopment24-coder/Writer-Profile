...
import { ShoppingCart, Sparkles, ArrowRight, FileText, Eye, Heart } from 'lucide-react';
    ...
    <div className="space-y-16 pb-16 animate-fade-in">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 min-h-[180px]">
            {displayNewReleases.map((book) => {
              ...
              return (
                <div
                  key={book.id}
                  className="bg-white p-5 rounded-2xl border border-[#E6E2D8] hover:shadow-xl transition-shadow duration-300 flex flex-col sm:flex-row gap-4 sm:gap-5 group min-h-[200px] overflow-hidden relative"
                >
                  <div 
                    className="w-28 h-36 shrink-0 rounded-lg overflow-hidden shadow-md cursor-pointer bg-[#EFECE6] relative aspect-[2/3] mx-auto sm:mx-0"
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between py-1 overflow-hidden">
                    <div className="space-y-1 min-w-0 pr-6 sm:pr-0">
                      <div className="flex items-center justify-between gap-1">
                        <span className="text-[10px] font-bold uppercase text-[#C29B47] truncate">
                          {book.category}
                        </span>
                        {onToggleWishlist && (
                          <button
                      ...
                      <h4 
                        onClick={() => onSelectBook(book)}
                        className="font-serif-bn font-bold text-base sm:text-lg text-[#1D1E20] group-hover:text-[#C29B47] cursor-pointer transition-colors leading-snug truncate whitespace-nowrap overflow-hidden text-ellipsis"
                        title={book.title}
                      >
                        {book.title}
                      </h4>
                    ...
                    <div className="pt-3 flex items-center justify-between gap-2 border-t border-[#F2EFE9] mt-2">
                      <div className="flex flex-col shrink-0">
                        <div className="flex items-baseline gap-1">
                          <span className="font-serif-bn font-bold text-base text-[#1D1E20]">৳{book.price}</span>
                          {book.originalPrice && book.originalPrice > book.price && (
                            <span className="text-[11px] text-[#8C887B] line-through font-serif-bn">৳{book.originalPrice}</span>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-1.5 shrink-0">
                        {book.pdfUrl && (
                          <button
                            ...
                            className="px-2.5 py-2 min-h-[38px] bg-[#F9F8F5] border border-[#D9D3C7] text-[#1D1E20] text-xs font-bold rounded-xl hover:bg-[#EFECE6] flex items-center gap-1 active:scale-95 transition-colors whitespace-nowrap"
                          >
                        ...
                        <button
                          ...
                          className="px-3.5 py-2 min-h-[38px] bg-[#1D1E20] hover:bg-[#C29B47] text-white text-xs font-bold rounded-xl shadow transition-colors flex items-center gap-1 active:scale-95 whitespace-nowrap"
                        >
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 min-h-[560px]">
        ...
        {homeDisplayBooks.length > 0 ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 min-h-[480px]">
            {homeDisplayBooks.map((book) => {
              const isWishlisted = wishlistIds.includes(book.id);
              return (
                <div
                  key={book.id}
                  className="bg-white border border-[#E6E2D8] rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-300 flex flex-col group h-full min-h-[480px]"
                >
                  <div 
                  ...
                  <div className="p-6 flex-1 min-w-0 flex flex-col justify-between space-y-4 overflow-hidden">
                    <div className="space-y-1.5 min-w-0">
                      ...
                      <h3 
                        onClick={() => onSelectBook(book)}
                        className="font-serif-bn font-bold text-xl text-[#1D1E20] group-hover:text-[#C29B47] cursor-pointer transition-colors leading-snug line-clamp-1 truncate whitespace-nowrap overflow-hidden text-ellipsis"
                        title={book.title}
                      >
                        {book.title}
                      </h3>
                    ...
                    <div className="pt-3 flex flex-wrap sm:flex-nowrap items-center justify-between gap-3 border-t border-[#E6E2D8]">
                      <div className="flex flex-col shrink-0">
