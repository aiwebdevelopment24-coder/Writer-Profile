...
export function updateSEOMeta({
  ...
  switch (view) {
    case 'home':
      ...
      break;
    case 'author':
      title = `আমার সম্পর্কে | ${authorName}`;
      description = `${authorName}-এর জীবনী, সাহিত্যচিন্তা ও পরিচিতি।`;
      break;
    case 'books':
    ...
    case 'single-book':
      if (selectedBook) {
        ...
        description = selectedBook.shortSynopsis ? selectedBook.shortSynopsis.slice(0, 160) : `${selectedBook.title} বইটি কিনুন। মূল্য: ৳ ${selectedBook.price}`;
      }
      break;
    case 'sample-reader':
      if (selectedBook) {
        title = `একটু পড়ুন: ${selectedBook.title} | ${authorName}`;
        description = `${selectedBook.title} বইটির ফ্রি নমুনা অনুলিপি।`;
      }
      break;
    case 'blog':
